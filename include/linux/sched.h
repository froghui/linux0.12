...
//TSS结构，TSS为存储在内存中的一段数据，其描述符在GDT中
struct tss_struct {
...
//这里n是第几个任务的意思，注意ldt, lldt后面跟的是GDT选择符，
//index == n<<4 + FIRST_XXX_ENTRY<<3 ; 而TI/RPL=000
...
//edx: 0x12345678  0x5678->addr+2/addr+3
//0x34->addr+4       0x12->addr+7
...
//limit为0-2^20 考虑到G=1 单位为4K，所以需要>>12
#define set_limit(ldt,limit) _set_limit( ((char *)&(ldt)) , (limit-1)>>12 )

//从段描述符addr(8 byte)中读取base
...
//获取当前进程代码段的limit数据， 单位为byte，注意这里是用lsll指令获取，会自动对G=4K进行转换，最终得到的值是以byte为单位，不是4K为单位
