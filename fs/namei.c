 ...
 */
 //比较dir entry里面的名字和name，输出为1，如果相同，否则输出为0，注意和C strncmp是相反的
static int match(int len,const char * name,struct dir_entry * de)
{
	...
	if (!len && (de->name[0]=='.') && (de->name[1]=='\0'))
		return 1;
	//dir 的name长度大于len，直接返回
	if (len < NAME_LEN && de->name[len])
		return 0;

	//下面的比较基于len==NAME_LEN或者 len<NAME_LEN && dir 的name的长度也是lenz
	__asm__("cld\n\t"
		"fs ; repe ; cmpsb\n\t"
		"setz %%al"
		:"=a" (same)
...
/* '..' over a mount-point results in 'dir' being exchanged for the mounted
   directory-inode. NOTE! We set mounted, so that we can iput the new dir */
	...
	while (i < entries) {
		//换block,一个block只能存放1024/16个dir_entry，所以一旦超过BLOCK_SIZE则需要换一个block
 ...
 */
 ...
 //对dir节点增加一个file entry字节点，主要是增大i_size,同时在目录块数据中写入文件名，但是文件所对应的inode指针并没用初始化
 //所以不能sleep，必须把buffer_head的数据完善，inode指针指向正确的值
...
int sys_mknod(const char * filename, int mode, int dev)
{
	...
	//新建一个inode，会从inode block中遍历找到可以用的一个
	inode = new_inode(dir->i_dev);
	if (!inode) {
		iput(dir);
		return -ENOSPC;
	...
	//dir entry的inode指向对应的inode index number
	de->inode = inode->i_num;
	bh->b_dirt = 1;
	iput(dir);
	iput(inode);
